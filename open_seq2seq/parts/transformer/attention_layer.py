...
class Attention(tf.layers.Layer):
  ...
  def call(self, x, y, bias, cache=None, last_positions=None, window_size=None):
    ...
    if self.mode == "loung":
      ...
      if dtype != tf.float32:
        ...
        if last_positions is not None:
          ...
          text_length = tf.shape(logits)[-1]
          mask_left = tf.sequence_mask(last_positions, maxlen=text_length, dtype=dtype) * NEG_INF
          mask_right = (1.0 - tf.sequence_mask(last_positions + window_size, maxlen=text_length, dtype=dtype)) * NEG_INF
          mask = mask_left + mask_right
          mask = tf.cast(mask, dtype=dtype)
          bias = mask + bias
          bias = tf.maximum(bias, 1.0 * NEG_INF)

        logits += bias
        weights = tf.nn.softmax(logits, name="attention_weights")

        if last_positions is not None:
          new_last_positions = tf.cast(tf.argmax(weights, axis=-1), dtype=tf.int32)
          new_last_positions = new_last_positions[:, :, -1:]
    ...
    if last_positions is not None:
      return attention_output, new_last_positions
